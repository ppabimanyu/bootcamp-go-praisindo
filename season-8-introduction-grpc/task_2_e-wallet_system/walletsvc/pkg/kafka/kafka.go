...
func New(config *KafkaConfig) *KafkaService {
	...
	if config.SecurityProtocol == "SCRAM_SHA_256" {
		...
		if err != nil {
			config.Logger.Error("can't setup kafka server", "error", err)
			os.Exit(1)
		}
		...
		config.Logger.Error("can't setup kafka server", "error", "invalid security protocol")
		os.Exit(1)
	}

	config.Logger.Debug(fmt.Sprintf("kafka security protocol %s", config.SecurityProtocol))
	config.Logger.Debug("kafka module initialized")
	...
	return &KafkaService{
		...
		errorLogger: kafka.LoggerFunc(func(message string, args ...interface{}) {
			config.Logger.Error(fmt.Sprintf(message, args...))
		}),
	}
}
